...
export interface ImportExportMap {
  ...
  exportIdToInfo: Map<string, { name: string; fileId: string; isDefault: boolean }>;
...
export function buildImportExportMap(
  ...
  allExports: Map<string, { id: string; name: string; fileId: string; isDefault: boolean }> = new Map(),
  currentPath: string = ""
): ImportExportMap {
  ...
  function collectFilesAndExports(
      ...
      fs.imports.forEach((imp) => {
        ...
        imp.imports.forEach((importName) => {
          ...
          if (importName === "default" || importName.startsWith("default")) {
            importedFile.exports.forEach((exp) => {
              if (exp.isDefault) {
            ...
            const hasReExport = importedFile.exports.some(exp => exp.name.startsWith("from '"));
            ...
            if (hasReExport) {
              ...
              importedFile.exports.forEach((exp) => {
                if (exp.name.startsWith("from '")) {
                  ...
                  const reExportCandidates = resolveImportPath(reExportPath, filePath);
                  ...
                  if (sourceFile && sourceFile.exports) {
                    ...
                    sourceFile.exports.forEach((sourceExp) => {
                      ...
                      const matches = 
                        ...
                        (sourceExp.isDefault && (importName === "default" || importName.startsWith("default")));
  ...
  function linkReExportsToImporters(fs: FileStructureWithId, path: string = "") {
    const filePath = path ? `${path}/${fs.name}` : fs.name;
    ...
    if (fs.type === "file" && fs.imports) {
      fs.imports.forEach((imp) => {
        ...
        imp.imports.forEach((importName) => {
          ...
          if (reExports) {
            reExports.forEach((reExport) => {
              const candidatePaths = resolveImportPath(reExport.from, importedFile!.name);
              ...
              sourceFile.exports.forEach((sourceExp) => {
                ...
                const matches = 
                  (reExport.exportNames === undefined || reExport.exportNames.length === 0) ||
                  reExport.exportNames.some(name => {
                    const originalName = name.includes(" as ") ? name.split(" as ")[0] : name;
                    return sourceExp.name === originalName;
                  });
                
                if (matches && (sourceExp.name === actualImportName || (sourceExp.isDefault && importName === "default"))) {
                  const sourceExportId = `${sourceFile.id}-export-${sourceExp.name}`;
